...
class App:  # Hauptanwendung mit Absprung in Unterprogromme
    ...
    def __init__(self):
        ...
        self.root.iconphoto(
            False, tk.PhotoImage(file=resource_path("kurzschlussanalyzer/images/blt_icon.png"))
        )
        ...
        # Menu Gestaltung
        left_menu_width = 150
        self.menu_left = tk.Frame(
            self.root, width=left_menu_width, bg="#ababab"
        )  # setzt die Hintergrundfarbe auf grau (ababab)
        ...
        self.test = tk.Label(
            self.menu_left_upper,
            text="Kurzschlussanalyzer",
        ...
        # Button für Dateiaufruf erstellen
        self.button_file_select = tk.Button(
            self.menu_left_upper, text="Explorer", command=self.__browse_files
        )
        ...
        self.test = tk.Label(
            self.menu_left_upper, text="SA, E: [A/ms]", font=("Segoe UI", 10, "normal")
        )
        ...
        self.test = tk.Label(
            self.menu_left_upper, text="SA, F: [A/ms]", font=("Segoe UI", 10, "normal")
        )
        ...
        self.test = tk.Label(
            self.menu_left_upper, text="SA, Tmax: [ms]", font=("Segoe UI", 10, "normal")
        )
        ...
        # Button für Neuberechnung
        self.button_update = tk.Button(
            self.menu_left_upper, text="Update", command=self.__update_calc
        )
    ...
    def __browse_files(self) -> None:
        ...
        self.__get_measurement_data()  # Funktionsaufruf Daten einlesen, DataFrame generieren
        ...
        self.entry_induktivitaet.delete(
            0, tk.END
        )  # löscht den aktuellen Inhalt des Eingabefelds
        self.entry_induktivitaet.insert(
            0, str(l_fl)
        )  # Schreibt den Wert von l_fl in das Eingabefeld 'induktivitaet'

        self.entry_widerstand.delete(
            0, tk.END
        )  # löscht den aktuellen Inhalt des Eingabefelds
        self.entry_widerstand.insert(
            0, str(r_fl)
        )  # schreibt den Wert von r_fl in das Eingabefeld 'widerstand'
        ...
        ddl_start, ddl_stop, ddl_trigger, trigger_type = safety_function(
            df_real,
            self.sa_e,
            self.sa_f,
            self.sa_delta_imax,
            self.sa_t_delta_imax,
            self.sa_tmax,
            self.sa_delta_imin,
        )
        print(trigger_type)
        ...
        print(
            f"Widerstand (r_fl): {r_fl} Ohm, Induktivität (l_fl): {l_fl} H, Tau: {tau} s"
        )
    ...
    def __get_measurement_data(self) -> None:
        ...
        if measurement_var == 1:
            ...
            for i, row in df.iterrows():
                ...
                next_voltage_value = (df.iloc[i + 1]).iloc[
                    2
                ]  # Spannungswert für die nächste Zeile
                current_value = row.iloc[1]  # Stromwert für die aktuelle Zeile
                next_current_value = (df.iloc[i + 1]).iloc[
                    1
                ]  # Stromwert für die nächste Zeile
        ...
        if measurement_var != 1:
            ...
            for i, row in df.iterrows():
                ...
                # Bedingungen überprüfen, um den Start zu markieren von der Messung
                if (current_value + 2.5) <= next_current_value or (
                    voltage_value - 2.5
                ) >= next_voltage_value:
                    currentindex = i
                    startflag = True
    ...
    def __create_plot(self, points, df_measure, df_real=pd.DataFrame) -> None:
        ...
        plot1.plot(
            df_measure["Time [s]"],
            df_measure["U Spannung [V]"],
            color="r",
            label="Kurzschlussspanung",
        )
        plot1.plot(
            df_measure["Time [s]"],
            df_measure["I Strom [A]"],
            color="g",
            label="Kurzschlusstrom",
        )
    ...
    def __update_calc(self) -> None:
        # neuaufruf
        r_fl, l_fl, tau, size_df = calculate(
            self.df_measure
        )  # Funktionsaufruf berechnungen

        self.__read_entrydata()
        df_real = real_current(
            size_df, self.l_fl, self.r_fl, self.df_measure, self.u_last
        )  # Funktionsaufruf realer kurzschluss berechnen
    ...
    def __update_status_from_trigger_type(self, trigger_type):
        if trigger_type == 1:
            ...
            self.__update_status(
                "keine Auslösung, Anstieg < F, innerhalb der Verzögerung"
            )
            ...
            self.__update_status(
                "keine Auslösung, delta T nicht vor F unterschreitung erreicht"
            )
            print("status 6", self.__update_status)
        else:
